class MusicCtx {
	...
	static toKeyNum( nt ){
      ...
      nt = nt.substring(0,nt.length-1);
		for (var i=0; i < MusicCtx.notes.length; i++)
			if ( nt.toUpperCase()=== MusicCtx.notes[i].nm.toUpperCase() ) 
				return MusicCtx.notes[i].midi4 + (oct-4)*12;
	...
	static toScale( md ){
		if ( md===undefined ) return MusicCtx.modeScales[0];
		if ( md instanceof Array && md.length >= 8 ) 
			return md;
		md = md.trim().toUpperCase();
		for (var i=0; i < MusicCtx.modeScales.length; i++)
			if ( md ===  MusicCtx.modeScales[i].nm.toUpperCase() )
				return MusicCtx.modeScales[i].scale;
		console.log(`toScale: unrecognized mode ${md}`);
		return MusicCtx.modeScales[0].scale;		// default Ionian
	}
	...
	static toChord( chd, root ){
		if ( chd instanceof Array ) 
			return chd.map( x => x + root );
		
		for (var i=0; i < MusicCtx.chords.length; i++)
			if ( chd === MusicCtx.chords[i].nm )
				return MusicCtx.chords[i].nts.map( x => x + root );
			
		console.log(`toChord: unrecognized chord ${chd}`);
		return MusicCtx.chords[0].nts.map( x => x + root );		// default major triad
	}
	static chordName( chd ){
		if ( !(chd instanceof Array)) return "?";
		var rt = chd[0], nt = '';
		if ( rt>0 ){
      chd = chd.map( x => x-rt );
      nt = MusicCtx.toString(rt);
    }
		for (var i=0; i < MusicCtx.chords.length; i++){
			if (chd.length === MusicCtx.chords[i].nts.length && 
        chd.every((val,idx) => val == MusicCtx.chords[i].nts[idx] ))
				return nt + MusicCtx.chords[i].nm;
    }
		return 'unrec chord';
	}
	...
	static toString( chd ){
		if ( chd instanceof Array ){
			...
			for ( var i=0; i < chd.length; i++ )
				s += MusicCtx.toString( chd[i] ) + ' ';
			return s + '] ' + MusicCtx.chordName(chd);
		}
    ...
    var keynum = parseFloat( chd );
      ...
      var scdeg = keynum % 12;
		  return MusicCtx.notes[scdeg].nm + (oct<0? '' : `${oct}`);		// treat -1 as no octave
		}
	...
	static test(){
      ...
      var kn = MusicCtx.toKeyNum(v);
 			console.log( `toKeyNum(${v}) = ${kn} = ${MusicCtx.toString(kn)}` );
    }

    for(const v of [ 'major', 'locrian', 'harmonic minor' ])
			console.log( `toScale(${v}) = [${MusicCtx.toScale(v)}]` );
    
    var m = new MusicCtx('C','major');
    var n = new MusicCtx('Eb','minor');
      
    for(const v of [ 'M', 'm', 'dim', 'mM7' ]){
			  var ch1 = MusicCtx.toChord(v, 'C');
			  var ch3 = MusicCtx.toChord(v,'E');
			  var ch4 = MusicCtx.toChord(v,'Bb');
 			  console.log( `toChord(${v},C) = [${ch1}] = ${MusicCtx.toString(ch1)} = ${MusicCtx.chordName(ch1)}` );
			  console.log( `toChord(${v},E) = [${ch3}] = ${MusicCtx.toString(ch3)} = ${MusicCtx.chordName(ch3)}` );
			  console.log( `toChord(${v},Bb) = [${ch4}] = ${MusicCtx.toString(ch4)} = ${MusicCtx.chordName(ch4)}` );
      }
    
	}
	
  constructor( root, mode ){
		this._root = MusicCtx.toKeyNum(root);
		this._scale = MusicCtx.toScale( mode );
  }
};
module.exports.MusicCtx = MusicCtx;